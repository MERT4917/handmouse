...
pyautogui.FAILSAFE = False  # köşeye gidince FailSafeException atmasın
pyautogui.PAUSE = 0          # her mouse çağrısındaki varsayılan 0.1s gecikmeyi kaldır
    ...
    if results.multi_hand_landmarks:
        ...
        if dist_thumb_index < THRESH_MOVE:
            if not tracking_mouse:
                ...
                mouse_start_pos = np.array(pyautogui.position(), dtype=np.float64)
                tracking_mouse = True
            else:
        ...
        now = time.time()
        if dist_thumb_ring < THRESH_CLICK and not tracking_mouse and (now - last_click_time) > 0.5:
            pyautogui.click()
            last_click_time = now
